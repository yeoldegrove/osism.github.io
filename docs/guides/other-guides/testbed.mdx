...
3. Configure your cloud access profile
   ...
   REGIO.cloud is used as an example here. The cloud name in `clouds.yaml`
   and the environment name (value of `ENVIRONMENT`) are `regiocloud` in this case. If another cloud
   is used, replace `regiocloud` with the respective profile name [from the table above](#cloud-access).
...
7. Deploy all services.
   ...
   so that deployments do not have to be lengthy. Run this script **before** the deploy scripts.

   You can prepare some basic OpenStack resources like public network, flavors and images by running
   ...
   `/opt/configuration/scripts/check.sh`. This step will take some time.
   ...
   Depending on the cloud, the installation will take some time. Up to two hours is not unusual. 
...
8. After the deployment, you can log in to the manager via SSH.

   ```sh
   make ENVIRONMENT=regiocloud
   ```
